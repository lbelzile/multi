...
# moyenne quadratique de validation
reqm_seq_ve_list <- 
...
plot(lasso_vc)
# Meilleur modèle
mod_lasso_vc <- which.min(lasso_vc$results$RMSE)
reqm_lasso_vc <- min(lasso_vc$results$RMSE)
nvar_lasso_vc <- Matrix::nnzero(lasso_vc$finalModel$beta[, mod_lasso_vc, drop = FALSE])
## Calcul des prédictions
# predict(cv_eqm_seq, newdata = db)
```
...
resultats <-data.frame(
  erreur = rep(c("validation croisée",
                  "validation externe"),
                  each = 2),
  method = rep(c("sequentielle","lasso"), 
               length.out = 4),
  reqm = c(reqm_seq_vc,
           reqm_lasso_vc,
           reqm_seq_ve,
