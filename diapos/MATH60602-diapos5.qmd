...
data(dbm, package = "hecmulti")
dbm_a <- dbm |>
  dplyr::filter(test == 0, #données d'entraînement
                !is.na(ymontant)) 
...
- estimer la performance (erreur quadratique moyenne) avec validation croisée ou critères d'information.
...
L'algorithme par séparation et évaluation (_branch and bound_) recherche de manière efficace sans essayer tous les modèles candidats.
...
Soit $\mathsf{E}(Y \mid \mathbf{X}) = f(\mathrm{X}_1, \ldots, \mathrm{X}_p)$ pour $f$ une fonction inconnue supposée lisse.
...
Le modèle est clairement **surajusté** avec `r length(coef(mod_complet))` coefficients pour `r nrow(dbm_a)` variables.
...
#| echo: true
library(glmnet)
lambda_seq <- seq(from = 0.1, to = 2, by = 0.01)
cv_output <- 
...
Une fois la valeur de $\lambda$ choisie, on réestime le modèle avec la pénalité et on calcule l'erreur quadratique moyenne.
