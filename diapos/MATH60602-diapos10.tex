...
\begin{document}
\frame{\titlepage}
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[sharp corners, borderline west={3pt}{0pt}{shadecolor}, enhanced, frame hidden, boxrule=0pt, interior hidden, breakable]}{\end{tcolorbox}}\fi
...
\item
  ...
  des composantes (ces poids sont appelés chargements). On peut
  distinguer des directions générales qui représentent l'orientation: si
  des variables pointent dans la même direction, elles sont typiquement
  fortement corrélées.
\end{itemize}
\end{frame}
