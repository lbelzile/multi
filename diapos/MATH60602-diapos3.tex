...
\item
  nécessite toutes les variables pour créer des composantes principales,
  incluant pour de nouvelles observations
\item
  le critère d'optimisation ne prend pas en compte une potentielle
  variable réponse comme pour l'analyse canonique et les moindres carrés
  partiels.
\item
  les composantes principales ne sont pas facilement identifiables
\item
  le modèle sert typiquement à des fins d'analyse exploratoire
  (visualisation) ou pour créer une variable réponse
\end{itemize}
\end{frame}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{facto\_cp }\OtherTok{\textless{}{-}}\NormalTok{ hecmulti}\SpecialCharTok{::}\FunctionTok{factocp}\NormalTok{(}\AttributeTok{x =}\NormalTok{ factor, }
