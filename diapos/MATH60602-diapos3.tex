...
\phantomsection\label{moduxe8les-pruxe9dictifs}
...
\phantomsection\label{rappels-sur-la-ruxe9gression-linuxe9aire}
...
\phantomsection\label{repruxe9sentation-alternative}
...
\phantomsection\label{postulats}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{ruxe9gression-linuxe9aire-en-deux-dimensions}
...
\begin{center}
\includegraphics[width=0.75\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos3_files/figure-beamer/unnamed-chunk-1-1.pdf}
\end{center}
\end{frame}

\begin{frame}{Résidus ordinaires}
\phantomsection\label{ruxe9sidus-ordinaires}
...
\phantomsection\label{illustration-des-ruxe9sidus-ordinaires}
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos3_files/figure-beamer/distancevert-1.pdf}
\end{center}
\end{frame}

\begin{frame}{Erreur quadratique moyenne}
\phantomsection\label{erreur-quadratique-moyenne}
...
\phantomsection\label{estimation-des-paramuxe8tres}
...
\phantomsection\label{estimation-dans-r}
...
\phantomsection\label{sortie-de-summary}
\footnotesize
...
\phantomsection\label{tableau-de-sortie}
\begin{itemize}
\tightlist
\item
  Formule de l'appel
...
\phantomsection\label{quelques-muxe9thodes-pour-lm}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{variables-catuxe9gorielles}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{encodage-des-variables-catuxe9gorielles}
...
\phantomsection\label{suxe9lection-de-variables-et-de-moduxe8les}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{pruxe9diction-vs-infuxe9rence}
\textbf{Prédiction}: obtenir la prédiction de \(\widehat{Y}\): on veut
...
\phantomsection\label{spuxe9cification-aduxe9quate-dun-moduxe8le}
\begin{block}{Démonstration \textbf{R}}
\phantomsection\label{duxe9monstration-r}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{uxe9valuer-la-performance-dun-moduxe8le}
...
\phantomsection\label{exemple-avec-la-ruxe9gression-polynomiale}
...
\begin{center}
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos3_files/figure-beamer/unnamed-chunk-6-1.pdf}
\end{center}
...
\phantomsection\label{ajustement-de-polynuxf4mes}
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos3_files/figure-beamer/unnamed-chunk-7-1.pdf}
\end{center}
...
\phantomsection\label{estimation-de-lerreur-quadratique-moyenne}
\begin{center}
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos3_files/figure-beamer/unnamed-chunk-9-1.pdf}
\end{center}
...
\phantomsection\label{suroptimisme}
...
\phantomsection\label{surajustement}
...
\begin{center}
\includegraphics[width=0.7\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos3_files/figure-beamer/overfitting-1.pdf}
\end{center}
\end{frame}

\begin{frame}{Morale de l'histoire}
\phantomsection\label{morale-de-lhistoire}
...
\phantomsection\label{estimation-fiable-de-la-performance}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{puxe9nalisation}
...
\phantomsection\label{lien-entre-ruxe9gression-linuxe9aire-et-vraisemblance}
...
\phantomsection\label{crituxe8res-dinformation}
Valables uniquement pour les modèles ajustés par \textbf{maximum de
vraisemblance}, ils sont de la forme

\begin{align*}
\mathsf{IC} &= \underset{-2 \times\text{log-vraisemblance}}{-2 \ell(\widehat{\boldsymbol{\theta}})} + \text{pénalité} \times\text{nb param} \\
\end{align*}
...
\item
  ...
  \(\ell(\widehat{\boldsymbol{\theta}})\) (qui indique la qualité de
  l'ajustement), donc minimiser le premier terme.
\item
...
\begin{frame}{Critères d'information}
\phantomsection\label{crituxe8res-dinformation-1}
...
\mathsf{BIC} &=n \ln (\widehat{\mathsf{EQM}}) +\ln(n)\times \text{nb param}+ \text{constante},
\end{align*}
...
\item
  La pénalité du \(\mathsf{BIC}\) augmente avec la taille de
  l'échantillon: \(\ln(100)\approx 4.6\), mais
  \(\ln(10 000) \approx 9.2.\)
\item
  Le \(\mathsf{BIC}\) (critère Bayésien de Schwarz) pénalise donc
  davantage que le \(\mathsf{AIC}\) (critère d'Akaike).
\item
  Possible de comparer des modèles avec des familles différentes (loi
  normale, ou GLM), mais il faut alors garder les constantes! Ce n'est
  pas toujours le cas avec les logiciels.
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Utiliser les critères d'information}
\phantomsection\label{utiliser-les-crituxe8res-dinformation}
\begin{itemize}
\tightlist
\item
...
\begin{frame}{Mises en garde}
\phantomsection\label{mises-en-garde}
Les fonctions \textbf{R} (ou dans d'autres langages de programmations)
omettent souvent le terme constant.

\begin{itemize}
\tightlist
\item
  Ce n'est pas grave si on utilise un même modèle (par ex., régression
  linéaire) pour différentes combinaisons de variables explicatives, car
  cette constante est la même.
\item
  C'est en revanche un problème si on compare des familles différentes
  (par exemple, modèle de Poisson vs régression linéaire): il faut
  conserver les constantes pour que la comparaison soit valide.
\end{itemize}

La comparaison n'est pas non plus valide si on oublie d'ajuster pour
toute transformation de la variable réponse, par exemple si on compare
un modèle avec \(\ln(y)\) vs un pour \(y\)!

\footnotesize

Technique: dans ce dernier cas, la fonction de densité inclut le
Jacobien de la transformation et cette dernière est incluse dans la
log-vraisemblance.
...
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos3_files/figure-beamer/fig-polynome-ajustement-1.pdf}
...
\phantomsection\label{suxe9paration-des-donnuxe9es}
...
\phantomsection\label{validation-externe}
...
\phantomsection\label{avantages-et-inconvuxe9nients-de-la-validation-externe}
...
\phantomsection\label{illustration-de-la-validation-croisuxe9e}
\begin{center}
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos3_files/figure-beamer/validationcroiseeillust-1.pdf}
\end{center}
\end{frame}

\begin{frame}{Étapes de la validation croisée à \(M\) groupes}
\phantomsection\label{uxe9tapes-de-la-validation-croisuxe9e-uxe0-m-groupes}
\begin{enumerate}
\tightlist
\item
...
\phantomsection\label{combien-de-groupes-m-pour-la-validation-croisuxe9e}
...
\phantomsection\label{validation-croisuxe9e-ruxe9sultat-aluxe9atoire}
...
\phantomsection\label{ruxe8gle-dune-erreur-type}
...
\phantomsection\label{illustration-de-la-ruxe8gle-de-une-erreur-type}
\pandocbounded{\includegraphics[keepaspectratio]{figures/fig-validationcroisee-une-erreur-type.pdf}}

La bande jaune donne une erreur-type du meilleur modèle. On choisirait
le modèle avec \(K=2\), plutôt que \(K=4\)
\end{frame}

\begin{frame}[fragile]{Validation croisée en \textbf{R}}
\phantomsection\label{validation-croisuxe9e-en-r}
...
\phantomsection\label{ruxe9sultats-de-la-validation-croisuxe9e}
\begin{figure}

\centering{

\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos3_files/figure-beamer/fig-plotcv-1.pdf}
...
\begin{frame}{Une note sur le coefficient de détermination}
\phantomsection\label{une-note-sur-le-coefficient-de-duxe9termination}
Le modèle de base avec uniquement l'ordonnée à l'origine \(\beta_0\)
retourne la même prédiction pour toutes les observations: la moyenne
\(\overline{y}.\) On dénote
\[\mathsf{EMQ}_0=\frac{\{(y_1-\overline{y})^2 + \cdots (y_n - \overline{y})^2\}}{n} = \frac{1}{n}\sum_{i=1}^n (y_i - \overline{y})^2\]
pour l'erreur quadratique moyenne du modèle de référence.

La performance relative d'un modèle par rapport à la référence es

\[R^2 = 1 - \frac{\mathsf{EMQ}}{\mathsf{EMQ}_0}.\]
\end{frame}

\begin{frame}{Relation entre \(R^2\) et erreur quadratique moyenne}
\phantomsection\label{relation-entre-r2-et-erreur-quadratique-moyenne}
Si on minimise l'erreur quadratique moyenne, on maximise le score
relatif.

Un meilleur modèle a ainsi un score \(R^2\) qui approche de \(1,\)
tandis qu'un modèle avec une valeur de \(0\) fait aussi bien que le
modèle de référence. Une valeur négative indique une performance pire
que ce dernier.

Comme pour l'erreur quadratique moyenne, ce critère n'est fiable que
s'il est évalué sur des prédictions d'observations qui n'ont pas servi à
l'estimation des paramètres.

Pour une comparaison adéquate, les données qui servent à estimer la
référence \(\mathsf{EMQ}_0\) doivent être les mêmes.
\end{frame}
