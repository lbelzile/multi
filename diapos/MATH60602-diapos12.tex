...
\protect\hypertarget{segmentation}{}
...
\tightlist
\item
  les étapes des regroupements
\item
  les avantages et inconvénients des méthodes
\item
  les conditions pour lesquelles la segmentation risque d'être
  applicable.
\end{enumerate}
\end{frame}
...
La probabilité qu'une observation soit tirée du groupe \(k\) est
\(\pi_k\).
\end{frame}
...
Un sous-indice \(k\) spécifie que cette composante varie d'un
...
modèle selon la covariance. Tirée de
...
\begin{frame}[fragile]{Hyperparamètres}
...
\begin{frame}{Sélection des hyperparamètres}
...
\item
  (\(-\)) le nombre de paramètres des matrices de covariance augmente
  rapidement avec le nombre de variables explicatives \(p\).
\end{itemize}
\end{frame}

\begin{frame}{Regroupements hiérarchiques}
\protect\hypertarget{regroupements-hiuxe9rarchiques}{}
Méthode \textbf{déterministe} de regroupement à partir d'une matrice de
dissimilarité.
...
La méthode de Ward va fusionner les deux groupes qui font le moins
augmenter l'homogénéité.
\end{frame}
...
dissemblance \(n \times n\): coût de stockage quadratique ou
\(\mathrm{O}(n^2)\).

Généralement, le coût de calcul est au mieux quadratique et au pire
cubique en \(n\).
...
\item
  ...
  seule estimation)
\item
  (\(-\)) le coût de calcul est très élevé, avec une complexité
...
On indique si les paires d'observations sont dans le même groupe (\(1\))
ou un groupe différent (\(0\)).
...
(\(0,0\) ou \(1,1\)), d'où un indice de \(\textsf{Rand} = 0.66\)
...
\item
  Effectuer la segmentation et valider la qualité de cette dernière
  (interprétabilité, taille des groupes, homogénéité des regroupements).
\item
...
\item
  ...
  \tightlist
  \item
    avec des mégadonnées, la complexité (coût de calcul et de stockage)
    est un facteur important pour choisir la méthode.
  \item
