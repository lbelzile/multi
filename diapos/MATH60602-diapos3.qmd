...
**Prédiction**: obtenir la prédiction de $\widehat{Y}$: on veut un modèle performant
...
- On veut maximiser la log-vraisemblance $\ell(\widehat{\boldsymbol{\theta}})$ (qui indique la qualité de l'ajustement), donc minimiser le premier terme.
...
- La pénalité du $\mathsf{BIC}$ augmente avec la taille de l'échantillon: $\ln(100)\approx 4.6$, mais $\ln(10 000) \approx 9.2.$
- Le $\mathsf{BIC}$ (critère Bayésien de Schwarz) pénalise donc davantage que le $\mathsf{AIC}$ (critère d'Akaike).
- Possible de comparer des modèles avec des familles différentes (loi normale, ou GLM), mais il faut alors garder les constantes! Ce n'est pas toujours le cas avec les logiciels.
...
Les fonctions **R** (ou dans d'autres langages de programmations) omettent souvent le terme constant. 

- Ce n'est pas grave si on utilise un même modèle (par ex., régression linéaire) pour différentes combinaisons de variables explicatives, car cette constante est la même.
- C'est en revanche un problème si on compare des familles différentes (par exemple, modèle de Poisson vs régression linéaire): il faut conserver les constantes pour que la comparaison soit valide.

La comparaison n'est pas non plus valide si on oublie d'ajuster pour toute transformation de la variable réponse, par exemple si on compare un modèle avec $\ln(y)$ vs un pour $y$!

\footnotesize

Technique: dans ce dernier cas, la fonction de densité inclut le Jacobien de la transformation et cette dernière est incluse dans la log-vraisemblance.
...
ggplot(data = EQM_graph, 
  ...
  labs(x = "ordre du polynôme",
       y = "",
       subtitle = "") +
  scale_color_manual(values=MetBrewer::met.brewer("OKeeffe1", 2)) +
  scale_x_continuous(breaks = 0:10,
                     labels = as.character(0:10)) +
  theme_classic() +
  theme(legend.position="bottom",
...
ggplot(df, aes(x, y)) +
  ...
  scale_color_manual(values=MetBrewer::met.brewer("Hiroshige", 2)[2:1]) +
  scale_fill_manual(values=MetBrewer::met.brewer("Hiroshige", 2)[2:1]) +
  scale_y_continuous(
    breaks = 1:5,
    minor_breaks = NULL,
    expand = c(0,0),
...
La bande jaune donne une erreur-type du meilleur modèle. On choisirait le modèle avec $K=2$, plutôt que $K=4$
...
# Une note sur le coefficient de détermination


Le modèle de base avec uniquement l'ordonnée à l'origine $\beta_0$ retourne la même prédiction pour toutes les observations: la moyenne $\overline{y}.$ On dénote $$\mathsf{EMQ}_0=\frac{\{(y_1-\overline{y})^2 + \cdots (y_n - \overline{y})^2\}}{n} = \frac{1}{n}\sum_{i=1}^n (y_i - \overline{y})^2$$
...
# Relation entre $R^2$ et erreur quadratique moyenne

Si on minimise l'erreur quadratique moyenne, on maximise le score relatif.

Un meilleur modèle a ainsi un score $R^2$ qui approche de $1,$ tandis qu'un modèle avec une valeur de $0$ fait aussi bien que le modèle de référence. Une valeur négative indique une performance pire que ce dernier.

Comme pour l'erreur quadratique moyenne, ce critère n'est fiable que s'il est évalué sur des prédictions d'observations qui n'ont pas servi à l'estimation des paramètres.

Pour une comparaison adéquate, les données qui servent à estimer la référence $\mathsf{EMQ}_0$ doivent être les mêmes.
