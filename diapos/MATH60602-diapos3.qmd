...
\begin{align*}
\mathsf{IC} &= \underset{\text{log-vraisemblance}}{-2 \ell(\widehat{\boldsymbol{\theta}})} + \text{pénalité} \times\text{nb param} \\
\end{align*}
...
Les données de la variable réponse $y_1, \dots, y_n$ doivent être les mêmes pour que la comparaison tienne (donc pas de transformation, par ex. modéliser $\ln(y)$, sans inclure le Jacobien de la transformation le cas échéant). 
