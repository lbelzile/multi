...
![Datasaurus (12 jeux de données avec la même corrélation linéaire)](figures/DataSaurusDozen.pdf)
...
- **critère des valeurs propres de Kaiser**: valeurs propres supérieures à 1. 
...
- la standardisation (covariance ou corrélation) impacte le résultat
- nécessite toutes les variables pour créer des composantes principales, incluant pour de nouvelles observations
- le critère d'optimisation ne prend pas en compte une potentielle variable réponse comme pour l'analyse canonique et les moindres carrés partiels.
- les composantes principales ne sont pas facilement identifiables
- le modèle sert typiquement à des fins d'analyse exploratoire (visualisation) ou pour créer une variable réponse
...
- composantes principales (garder $m$ vecteurs propres et valeurs propres)
   - estimation toujours valide et rapide.
   - sélection moins objective.
- **maximum de vraisemblance**: postulat de normalité, nécessite une optimisation numérique délicate pour chaque $m$
   - critères d'information.
   - problèmes de convergence fréquents (variances estimées négatives!) appelés cas de **(quasi)-Heywood**.
...
library(hecmulti)
ajustement_factanal(
    covmat = corr(factor), # matrice de corrélation
...
---
# Ajustement avec composantes principales

```{r}
#| label: facto-cpkaiser
#| eval: false
#| echo: true
facto_cp <- hecmulti::factocp(x = factor, 
                  nfact = "kaiser")
```

# Solution avec composantes principales
