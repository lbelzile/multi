...
ordinales à cotes proportionnelles. }\tabularnewline
...
cst [rarement/jamais|occas.] & -1.297 & 0.044\\
cst [occas.|toujours] & 0.865 & 0.041\\
\bottomrule
\end{tabular}
\end{table}
...
\frac{\Pr(Y = \texttt{rarement} \mid \texttt{sexe})}{\Pr(Y \geq \texttt{occas.} \mid \texttt{sexe})} &= \exp(-0.166\texttt{sexe} + 1.297) \\ 
\frac{\Pr(Y \leq \texttt{occas.} \mid \texttt{sexe})}{\Pr(Y = \texttt{toujours} \mid \texttt{sexe})} &= \exp(-0.166\texttt{sexe} - 0.865).
\end{align*}
\end{frame}
