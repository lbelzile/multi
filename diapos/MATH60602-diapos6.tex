...
\phantomsection\label{pruxe9diction}
...
\phantomsection\label{classification-de-base}
...
\phantomsection\label{qualituxe9-de-lajustement}
...
\begin{frame}{Score de Brier}
\phantomsection\label{score-de-brier}
Le score propre de Brier pour la prédiction \(\widehat{p}_i\) de la
probabilité de succès avec des variables binaires \(y_i \in \{0, 1\}\)
\begin{align*}
\mathsf{SB} = \frac{1}{n} \sum_{i=1}^n (\widehat{p}_i - y_i)^2.
\end{align*} Une petite valeur implique une meilleure performance.

Ce score prend en compte l'aspect probabiliste de la prédiction.
\end{frame}

\begin{frame}[fragile]{Base de données marketing}
\phantomsection\label{base-de-donnuxe9es-marketing}
...
\phantomsection\label{estimation-de-la-performance-du-moduxe8le}
...
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos6_files/figure-beamer/figclassification0-1.pdf}
\end{center}
...
\phantomsection\label{choix-dun-point-de-coupure.}
...
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos6_files/figure-beamer/unnamed-chunk-3-1.pdf}
\end{center}
...
\phantomsection\label{performance-et-classification}
\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\NormalTok{classif }\OtherTok{\textless{}{-}}\NormalTok{ appr}\SpecialCharTok{$}\NormalTok{yachat}
...
optimal. Les entrées donnent le décompte du nombre d'observation par
combinaison \((Y, \widehat{Y})\)}
...
\phantomsection\label{classification-et-mesures-de-performance}
\footnotesize
...
\phantomsection\label{matrice-de-gain}
...
\caption{\label{tbl-tableconfumat4}Pondération des entrées de la matrice
de confusion pour le calcul du gain.}
...
\phantomsection\label{gain-pour-taux-de-bonne-classification}
Si on cherche à maximiser le taux de bonne classification, cela revient
à assigner les poids suivants.

\begin{table}

\caption{\label{tbl-tableconfumat3}Pondération des entrées de la matrice
de confusion pour le calcul du taux de bonne classification.}
...
\phantomsection\label{couxfbts-et-buxe9nuxe9fices-du-ciblage-marketing}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{matrice-de-gain-pour-ciblage-marketing}
\begin{table}

\caption{\label{tbl-tableconfumat5}Matrice de gain pour ciblage
marketing.}
...
\phantomsection\label{point-de-coupure-avec-gain}
\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{60602}\NormalTok{)}
...
\phantomsection\label{calcul-du-gain}
\begin{figure}

\centering{

\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos6_files/figure-beamer/fig-coupure-pondere-1.pdf}
...
\phantomsection\label{point-de-coupure-optimal-et-explication}
...
\phantomsection\label{fonction-defficacituxe9-du-ruxe9cepteur}
...
\phantomsection\label{courbe-roc}
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos6_files/figure-beamer/unnamed-chunk-6-1.pdf}
\end{center}
\end{frame}

\begin{frame}{Aire sous la courbe}
\phantomsection\label{aire-sous-la-courbe}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{courbe-lift}
...
Généralement, le lift est calculé en fonction du budget (par exemple, si
on a 10 000 clients et qu'on a du budget pour envoyer une offre
uniquement à 1000 clients, on prendrait \(x\)\%\(=0.1\).

\normalsize
\end{frame}

\begin{frame}[fragile]{Code pour produire la courbe lift}
\phantomsection\label{code-pour-produire-la-courbe-lift}
...
\phantomsection\label{tableau-du-lift}
\begin{table}
...
\phantomsection\label{calcul-du-lift}
...
\begin{frame}{Courbe lift}
\phantomsection\label{courbe-lift-1}
\pandocbounded{\includegraphics[keepaspectratio]{MATH60602-diapos6_files/figure-beamer/figcourbe-lift-pr-1.pdf}}
\end{frame}

\begin{frame}{Calibration du modèle}
\phantomsection\label{calibration-du-moduxe8le}
Certains modèles sont trop confiants dans leurs prédictions
(surajustement). Pour une variable réponse binaire \(Y \in \{0,1\}\), le
score de Brier (l'erreur quadratique moyenne) est décomposé comme suit:
\begin{align*}
\mathsf{BS} &= \underset{\text{score de Brier}}{\frac{1}{n} \sum_{i=1}^n (Y_i-p_i)^2}
...
\phantomsection\label{test-de-spiegelhalter}
...
\begin{center}
\includegraphics[width=0.6\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos6_files/figure-beamer/unnamed-chunk-9-1.pdf}
\end{center}
...
\phantomsection\label{suxe9lection-de-variables-en-ruxe9gression-logistique}
\begin{itemize}
\tightlist
\item
...
modèles}
\phantomsection\label{fonctions-r-pour-la-suxe9lection-de-moduxe8les}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{objectif-du-ciblage-marketing}
...
\phantomsection\label{stratuxe9gie-de-ruxe9fuxe9rence}
\begin{itemize}
\tightlist
\item
...
\phantomsection\label{stratuxe9gie-dajustement}
...
\phantomsection\label{pruxe9diction-et-envoi}
\begin{itemize}
\tightlist
\item
...
dégagé des revenus nets de \(978 226\)\$.
