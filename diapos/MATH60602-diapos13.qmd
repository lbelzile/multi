...
- On ne peut les ignorer (étude des cas complets) sans biaiser les interprétations et réduire la quantité d'information disponible.
- Pour bien capturer l'**incertitude** et ne pas modifier les relations entre variables, il faut utiliser une méthode **stochastique**.
- Avec l'algorithme MICE, on utilise un modèle conditionnel pour chaque variable à tour de rôle

# Récapitulatif

L'imputation multiple est préférée à l'imputation simple car elle permetd'estimer l'incertitude sous-jacente en raison des données manquantes.
