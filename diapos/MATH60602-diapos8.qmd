...
La survie $S_{ij}$ est la probabilité que $Y_i$ soit plus grand ou égal à $j$,
\begin{align*}
...
On considère la cote de $S_{ij}$ (versus $1-S_{ij}$), qui mesure à quel point il est plus probable que $Y_i$ prenne une valeur supérieure ou égale à $j$ par rapport à une valeur inférieure à $j$, avec
\begin{align*}
...
Pour chaque augmentation d'une unité de $\mathrm{X}_j$, la cote $S_k$ versus $(1-S_k)$ est multipliée par $\exp(\beta_j)$, peu importe la valeur de $Y$. 
...
tabcoef <- cbind("effet" = c("sexe [homme]","cst [rarement/jamais|sporadiquement]","cst [sporadiquement|toujours]"), tabcoef)
...
Les hommes de plus de 30 ans sont plus susceptibles de voter fréquemment que les femmes. 
...
# Postulat de rapport de cote constant
