...
On considère une variable catégorielle avec $k \ge 2$ niveaux.

**Objectif**: modéliser la probabilité de chaque catégorie de la variable réponse.

- Comme avec la régression logistique, on fixe une catégorie de référence (étiquetée `0`).
- On modélise le log de la cote (logit) de chacune des autres catégories par rapport à la référence.
\begin{align*}
...
\end{align*}
où $p_{ik} = \Pr(Y_i=k \mid \mathrm{X}_i)$ $(k=0, \ldots, K)$. 

# Modèle logistique multinomial et probabilités 

On peut exprimer le modèle en termes des probabilités,
\begin{align*}
 p_{i0} &= \Pr(Y_i=0 \mid \mathrm{X}_i) = \frac{1}{1+ \sum_{j=1}^K\exp(\beta_{0j}+\beta_{1j}\mathrm{X}_i)}\\
 p_{ik} &= \Pr(Y_i=k \mid \mathrm{X}_i) = \frac{\exp(\beta_{0k}+\beta_{1k}\mathrm{X}_i)}{1+ \sum_{j=1}^K\exp(\beta_{0k}+\beta_{1k}\mathrm{X}_i)}, \qquad k =1, \ldots, K.
\end{align*}

- La  somme des probabilités, $p_{i0} + \cdots + p_{iK}$, vaut 1.
- Le modèle logistique multinomial ne fait que combiner plusieurs logit dans un seul modèle.
- L'interprétation des paramètres se fait comme en régression logistique sauf qu'il faut y aller équation par équation.
- Avec $K + 1$ niveaux et $p$ variables explicatives, on obtiendra $(K+1)\times (p+1)$ paramètres à estimer, en incluant l'ordonnée à l'origine.
