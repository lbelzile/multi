...
set.seed(202209)
# Option par défaut: 
# validation croisée à 10 plis, 10 répétitions
...
perfo1 <- perfo(prob = prob, resp = appr$yachat)
```
...
Si on classifiait comme acheteurs les $10$\% qui ont la plus forte probabilité estimée d'achat, on détecterait $78$ des $210$ clients.
...
#| eval: true
#| label: tbl-quasisep
#| tbl-cap: "Données simulées avec séparation de variable pour modèle logistique (gauche) et correction de Firth (droite)."
#| warning: false
#| message: false
set.seed(1234)
x <- sort(rexp(20, rate = 0.2))
...
rownames(tab1) <- c("cst","x")
tab2 <- data.frame(estimate = as.numeric(m2$coef), "std.error" = as.numeric(sqrt(diag(m2$var))))
colnames(tab2) <- c("coef.", "erreur-type")
tab2 <- apply(tab2, 1:2, function(x){paste0("$", sprintf(x, fmt = "%.1f"), "$")})
rownames(tab2) <- c("cst","x")
knitr::kable(x = list(tab1, tab2), align = "rrr", booktabs = TRUE, escape = FALSE)
```
...
3. Si on a des problèmes de convergence numérique ($1$e message d'erreur), on peut aussi ajouter une pénalité, proposée par Firth (1993) pour empêcher les valeurs des coefficients d'être trop élevés (un peu comme le LASSO). 
    - Dans **R**, via la fonction `logistf` du paquet éponyme.
