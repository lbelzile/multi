...
La fonction `glm` dans **R** ajuste un modèle linéaire généralisé (par défaut, Gaussien)

- L'argument `family=binomial(link="logit)` permet de spécifier que l'on ajuste un modèle logistique


# Modèle avec une seule variable
...
Significativité globale avec test du rapport de vraisemblance vs $\mathscr{H}_0: p_i = \mathrm{expit}(\beta_0)$
...
On peut fixer la valeur de $\beta_j$ et maximiser la vraisemblance pour les autres paramètres.
