...
Le score propre de Brier pour la prédiction $\widehat{p}_i$ de la probabilité de succès avec des variables binaires $y_i \in \{0, 1\}$
\begin{align*}
\mathsf{SB} = \frac{1}{n} \sum_{i=1}^n (\widehat{p}_i - y_i)^2.
\end{align*}
Une petite valeur implique une meilleure performance.
...
Généralement, le lift est calculé en fonction du budget (par exemple, si on a 10 000 clients et qu'on a du budget pour envoyer une offre uniquement à 1000 clients, on prendrait $x$%$=0.1$.
...
Certains modèles sont trop confiants dans leurs prédictions (surajustement). Pour une variable réponse binaire $Y \in \{0,1\}$, le score de Brier (l'erreur quadratique moyenne) est décomposé comme suit:
\begin{align*}
...
<!--  $$Z = \frac{\mathsf{BS}-\mathsf{E}_0(\mathsf{BS})}{\mathsf{sd}_0(\mathsf{BS})}$$  -->
