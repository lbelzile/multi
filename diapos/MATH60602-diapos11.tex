...
l'objectif est de déduire les regroupements sans étiquette préalable
...
\item
  ...
  les données.
\item
  Choisir un algorithme et ses hyperparamètres (dissemblance, nombre de
  regroupements, etc.)
\item
  Regrouper et étiqueter les observations.
\item
  Valider la qualité de la segmentation.
\item
  Obtenir le prototype de chaque groupe et interpréter les regroupements
  à l'aide de ces derniers.
\end{enumerate}
\end{frame}

\begin{frame}{Exemples d'analyses de regroupements}
\protect\hypertarget{exemples-danalyses-de-regroupements}{}
\begin{itemize}
\tightlist
\item
...
\NormalTok{donsmult }\OtherTok{\textless{}{-}}\NormalTok{ hecmulti}\SpecialCharTok{::}\NormalTok{dons }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{filter}\NormalTok{(ndons }\SpecialCharTok{\textgreater{}}\NormalTok{ 1L) }\SpecialCharTok{|\textgreater{}} \CommentTok{\#seulement dons multiples}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{mtdons =}\NormalTok{ vdons}\SpecialCharTok{/}\NormalTok{ndons, }\CommentTok{\# montant moyen}
         \CommentTok{\# nombre de dons selon l\textquotesingle{}anciennete}
...
\NormalTok{           npromesse }\SpecialCharTok{\textgreater{}} \DecValTok{0} \SpecialCharTok{\textasciitilde{}}\NormalTok{ vpromesse}\SpecialCharTok{/}\NormalTok{npromesse, }\CommentTok{\#montant moyen}
           \ConstantTok{TRUE} \SpecialCharTok{\textasciitilde{}} \DecValTok{0}\NormalTok{)) }\SpecialCharTok{|\textgreater{}} \CommentTok{\# zero si aucune promesse}
  \FunctionTok{select}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{c}\NormalTok{(}
\NormalTok{    vradiations, }\CommentTok{\# trop de valeurs manquantes dans vradations}
...
\NormalTok{    vpromesse, nrefus, nradiations)) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{relocate}\NormalTok{(mtdons) }\CommentTok{\# mettre mtdons dans la 1e colonne}
\end{Highlighting}
\end{Shaded}
\end{frame}

\begin{frame}{Corrélation des nouvelles données}
\protect\hypertarget{corruxe9lation-des-nouvelles-donnuxe9es}{}
\begin{figure}

{\centering \includegraphics[width=0.8\textwidth,height=\textheight]{MATH60602-diapos11_files/figure-beamer/unnamed-chunk-5-1.pdf}
...
\begin{frame}{Mesures de dissimilarité et de similarité}
\protect\hypertarget{mesures-de-dissimilarituxe9-et-de-similarituxe9}{}
Comment mesurer si deux observations appartiennent à un même
regroupement et sont similaires?

Une mesure de dissimilarité sert à quantifier la proximité de deux
objets à partir de leurs coordoonnées.

Plus la dissimilarité est élevée, moins les observations sont semblables
(plus éloignées).
\end{frame}

\begin{frame}{Mesures de dissimilarité}
\protect\hypertarget{mesures-de-dissimilarituxe9}{}
...
\item
  la dissimilarité est la même peu importe l'ordre des observations
  (symmétrie)
\end{enumerate}
...
La mesure de dissimilarité la plus utilisée en pratique est la distance
euclidienne.
...
\begin{frame}{Autres mesures de dissimilarité}
\protect\hypertarget{autres-mesures-de-dissimilarituxe9}{}
\begin{figure}

{\centering \includegraphics[width=0.7\textwidth,height=\textheight]{figures/distance.pdf}
...
\begin{frame}{Autres mesures de dissimilarité}
\protect\hypertarget{autres-mesures-de-dissimilarituxe9-1}{}
Pour les données catégorielles nominales, on peut assigner une
dissimilarité de 0 si les variables ont la même modalité et 1 sinon.
...
\begin{frame}[fragile]{dissimilarités dans \textbf{R}}
\protect\hypertarget{dissimilarituxe9s-dans-r}{}
...
triangulaire inférieure (puisque la distance est symmétrique).
\end{frame}
...
\item
  ou utiliser des mesures robustes: soustraire la médiane et diviser par
  l'écart absolu à la médiane (\emph{mad})
\end{itemize}
...
\item
  méthodes basées sur la densité (pas couvertes)
\end{enumerate}
\end{frame}

\begin{frame}{Notation pour la complexité}
\protect\hypertarget{notation-pour-la-complexituxe9}{}
\begin{itemize}
\item
  La notation \(\mathrm{O}(\cdot)\) nous renseigne sur l'ordre du nombre
  d'opérations nécessaires pour le calcul. Par exemple, additionner une
  colonne de chiffres nécessite \(\mathrm{O}(n)\) flops
\item
  De même, le stockage d'une matrice de distance, qui contient
  \(n(n-1)/2\) entrées distinctes, est \(\mathrm{O}(n^2)\).
\end{itemize}

Plus le chiffre (ou la puissance) est élevé, plus le calcul ou le
stockage est coûteux.
\end{frame}
...
\item
  ...
  rayon, choix de la dissimilarité, etc.) à spécifier selon les
  méthodes.
\end{itemize}
\end{frame}
...
On assigne chaque observation à un de \(K\) regroupements, représentés
par un prototype, disons \(\boldsymbol{\mu}_k\) pour le regroupement
\(k\).
...
minimiser la distance entre les observations et les prototypes.
\end{frame}
...
Quelques forces (\(+\)) et faiblesses (\(-\)) de l'algorithme des \(K\)
moyennes
...
\end{align*} est la somme des distances euclidiennes au carré entre les
observation du groupe \(G_k\) et leur barycentre \(\boldsymbol{\mu}_k\).

Avec un seul groupe, la distance par rapport à la moyenne est
\(\mathsf{SCT} = \mathsf{SCD}_{1}\), la somme totale du carré de toutes
les distances par rapport au barycentre global.
\end{frame}
...
Coûteux en calcul (nécessite matrice de dissimilarité), possible de
faire avec un sous-échantillon aléatoire.
...
{\centering \includegraphics[width=0.7\textwidth,height=\textheight]{MATH60602-diapos11_files/figure-beamer/fig-silhouette-1.pdf}
...
\item
  Plusieurs choix de l'analyste (mesure de dissimilarité, algorithme,
