...
Le modèle Bradley--Terry de base n'a pas d'ordonnée à l'origine, mais on peut généraliser le modèle en l'incluant. Cela ajoute un autre paramètre, $\alpha$, qui capture l'effet du jeu à domicile: si $Y_i$ est le score de l'équipe à domicile et $Y_j$ l'équation devient
$$\ln\left\{\frac{\Pr(\text{victoire équipe $i$ (domicile)})}{\Pr(\text{victoire équipe $j$ (visiteur)})}\right\}= \alpha + \beta_i - \beta_j,$$
où $\alpha$ est simplement le paramètre pour l'ordonnée à l'origine.
...
Ajustez le modèle de Bradley--Terry aux données `lnh_BT` (utilisez la formule `vainqueur ~ .` pour ajuster le modèle avec toutes les équipes). La catégorie de référence est `Anaheim Ducks`.
