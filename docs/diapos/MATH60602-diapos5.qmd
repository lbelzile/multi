...
On voit que $p$ est une **fonction nonlinéaire croissante** de $\eta=\beta_0 + \cdots + \beta_p \mathrm{X}_{p}$.
...
#| out.width: '80%'
#| fig-align: "center"
knitr::include_graphics("figures/cote.png")
```
...
\small 
Une étude de [Duke et Amir (2023)](doi:10.1287/mksc.2022.1364) dans *Marketing Science* considère l'utilisation d'un format de vente 
- `intégré` (choisir directement d'acheter et la quantité en une seule étape, versus 
- `séquentiel`, le standard actuel (séquentiel), qui revient à choisir d'acheter (oui ou non), puis sélectionner la quantité.
...
#| out.width: '100%'
#| fig-align: 'center'
knitr::include_graphics("figures/DukeAmir.png")
```
\normalsize

# Base de données `formatvente`

Les données de l'expérience 3 contiennent 399 observations des variables suivantes

- `format`: variable expérimental, soit `intégré` ou `séquentiel`
- `achat`: variable binaire, $1$ pour achat
- `quantite`: quantité en cas d'achat ($0$, $1$ ou $2$)
- `marque`:  nom de la première marque (pour un item vs deux), soit `for your beauty` ou `beauty care`, contrebalancé
- `genre`: genre, `masculin` ou `féminin` (4 valeurs manquantes)
- `age`: âge du répondant

# Données expérimentales

Comme les participants sont assignés de manière aléatoire à chaque sous-groupe de `format`, seule cette variable et l'ordre de la `marque` peuvent avoir une incidence.

Les variables sociodémographiques `genre` et `age` sont uniquement collectées pour vérifier la représentativité de l'échantillon et la généralisabilité des conclusions.
...
modele1 <- glm(formula = achat ~ format,
...
# Interprétation avec données de format de vente
...
Le modèle ajusté en terme de cote est
\begin{align*}
 \frac{\Pr(Y=1 \mid \mathrm{X}_1=1)}{\Pr(Y=0 \mid \mathrm{X}_1=x_1)} = \exp(`r round(as.numeric(coef(modele1)[1]), 3)`)\exp(`r round(as.numeric(coef(modele1)[2]),3)`).
\end{align*}

\small

- Lorsque qu'on passe d'un format intégré à un format séquentiel, la cote est multipliée par $\exp(`r round(as.numeric(coef(modele1)[2]),3)`) = `r round(exp(as.numeric(coef(modele1)[2])),3)`.$ C'est une diminution de la cote de $`r 100*(1-round(exp(as.numeric(coef(modele1)[2])),3))`$%. La probabilité d'achat est plus faible pour le format séquentiel que le format intégré.
...
L'estimateur du **maximum de vraisemblance** est la valeur du vecteur de paramètres qui maximise la fonction de vraisemblance. Ce sont les valeurs qui rendent les données les plus plausibles.
...
- par exemple, si on ajuste un modèle pour `achat` en fonction de `quantite`.
...
modele_qs <- glm(
  ...
  data = formatvente, 
  family = binomial)
fittedsepmod <- function(x){
  beta <- as.numeric(coef(modele_qs))
 plogis(beta[1] +  beta[2] * x)
}
library(ggplot2)
ggplot(data = formatvente, 
       mapping = aes(x = quantite, y = achat)) +
  geom_point() +
...
Quasi-séparation de variable: les estimations des paramètres sont presques infinies pour permettre une transition abrupte de la probabilité de succès de $\widehat{p}=0$ à $\widehat{p}=1$ à $x=0.5$.
...
#| eval: true
#| label: tbl-quasisep
#| tbl-cap: "Modèle logistique pour `achat` en fonction de `quantite`."
#| warning: false
#| message: false

tab1 <- data.frame(estimate = as.numeric(modele_qs$coefficients), "std.error" = as.numeric(sqrt(diag(vcov(modele_qs)))))
colnames(tab1) <- c("coef.", "erreur-type")
tab1 <- apply(tab1, 1:2, function(x){paste0("$", sprintf(x, fmt = "%.1f"), "$")})
rownames(tab1) <- c("cst","quantite")
...
- L'estimation par maximum de vraisemblance permet l'usage des tests d'hypothèse basés sur la déviance.
- Si on teste un seul paramètre, on peut également effectuer le test en regardant si la valeur postulée pour $\beta$ se trouve dans l'intervalle de confiance de vraisemblance profilée, qui est invariant aux reparamétrisations.
