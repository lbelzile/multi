...
\(2^p=2\times 2 \times \cdots \times 2\) (\(p\) fois) modèles. Au total,
\(2^{14} = 16384.\)
...
\begin{frame}[fragile]{Recherche efficace}
\phantomsection\label{recherche-efficace}
...
\begin{center}
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures/fig-perfo-sequentiel.pdf}
\end{center}

\footnotesize

Performance des 40 premiers modèles de la procédure séquentielle en
fonction du nombre de termes inclus. La méthode externe donne la vrai
...
\textbf{Solution}: standardiser les variables explicatives
\(\mathrm{X}_1, \ldots, \mathrm{X}_p\) \textbf{et} la variable réponse
\(\boldsymbol{y}\) pour qu'elles soient toutes à la même échelle (par
exemple, avec moyenne nulle et un écart-type unitaire).

\begin{itemize}
\tightlist
\item
  vérifier selon le logiciel, cette étape peut-être effectuée
  implicitement par votre fonction.
\end{itemize}
\end{frame}
...
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{figures/fig-lassopenalty.pdf}
\end{center}
\end{frame}
...
et le critère visant à choisir le meilleur modèle (le profit en
entreprise).
\end{frame}
