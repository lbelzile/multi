...
\begin{frame}{Étapes de la modélisation prédictive}
\phantomsection\label{uxe9tapes-de-la-moduxe9lisation-pruxe9dictive}
\begin{enumerate}
\tightlist
\item
  Préparation de la base de données (analyse exploratoire, nettoyage)
\item
...
\item
  ...
  \item
    Choix du modèle
  \item
    Choix des variables explicatives ou des hyperparamètres
  \end{enumerate}
\item
  Prédictions
\item
...
\item
  Sélection du modèle final, ré-estimation et prédiction
\end{enumerate}
\end{frame}

\begin{frame}{Présentation des données}
\phantomsection\label{pruxe9sentation-des-donnuxe9es}
On considère une base de données marketing. L'objectif d'affaires est de
cibler les clients à qui envoyer un échantillon promotionnel.
...
\tightlist
\item
  Envoyer un échantillon de produits au coût de 10\$ à un groupe pilote
  de 1000 personnes et mesurer le montant d'achat suite à l'envoi.
\item
  Construire un modèle de prédiction pour déterminer à qui envoyer le
  produit parmi tous les 100 000 clients résiduels.
\end{enumerate}

On supposera que, au final, chaque client a reçu l'échantillon. Le but
de l'exercice rétrospectif est de voir s'il aurait été possible de faire
mieux en ne ciblant que ceux susceptibles d'acheter.
\end{frame}

\begin{frame}{Population cible}
\phantomsection\label{population-cible}
Clients qui ont
...
\tightlist
\item
  1K personnes dans l'échantillon d'apprentissage (pilote),
\item
...
\phantomsection\label{liste-des-variables}
\footnotesize
...
\item
  ...
  acheté quelque chose suite à l'envoi, égale à 1 si oui et 0 sinon;
\item
...
\item
  \texttt{x6}: nombre d'année du lien d'affaires;
\item
...
\phantomsection\label{aperuxe7u-des-donnuxe9es}
\footnotesize
...
\begin{frame}
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos4_files/figure-beamer/histogrammes-eda-dbm-fig-1.pdf}
\end{frame}

\begin{frame}
\footnotesize

\begin{xltabular}{\linewidth}{>{\raggedright\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X>{\raggedleft\arraybackslash}X}
\toprule
...
\phantomsection\label{pruxe9dire-le-montant-ymontant}
...
Le modèle utilise les données des 210 personnes (sur 1000) de
l'échantillon d'apprentissage qui ont acheté suite à l'envoi de
l'échantillon.
\end{frame}

\begin{frame}[fragile]{Séparation de l'échantillon}
\phantomsection\label{suxe9paration-de-luxe9chantillon}
On conserve 100 000 observations test pour vérifier la performance

\begin{itemize}
\tightlist
\item
  (oracle): réponses inconnues à toutes fins pratiques en temps
  ordinaire.
\end{itemize}

Nous utiliserons la pénalisation ou la validation croisée pour choisir
notre modèle, en nous basant uniquement sur les 1000 observations du
projet pilote.
...
\phantomsection\label{recherche-dun-moduxe8le}
Idée initiale:
...
\item
  estimer pour chacun la performance (par ex., l'erreur quadratique
  moyenne) par validation croisée,
\item
  déterminer le meilleur modèle selon ce critère parmi les modèles
  candidats.
\end{itemize}
\end{frame}

\begin{frame}{Digression sur les variables catégorielles}
\phantomsection\label{digression-sur-les-variables-catuxe9gorielles}
Les algorithmes que l'on va utiliser ont parfois des comportements
différents face aux variables catégorielles (facteurs) avec \(K > 2\)
catégories, qui définit un ensemble de \(K-1\) variables binaires.

Soit le modèle considère l'ajout ou l'exclusion

\begin{itemize}
\tightlist
\item
  de chacune des \(K-1\) variables binaires créées,
\item
  de l'ensemble des modalités simultanément.
\end{itemize}

La première option permet plus de flexibilité, mais comme les catégories
retirées sont assimilées à l'ordonnée à l'origin, la catégorie de
référence n'est plus arbitraire.
\end{frame}

\begin{frame}[fragile]{Fléau de la dimensionalité}
\phantomsection\label{fluxe9au-de-la-dimensionalituxe9}
Combien de modèles incluant les combinaisons de \(p\) variables? Chaque
variable est incluse (ou pas): il y a
\(2^p=2\times 2 \times \cdots \times 2\) (\(p\) fois) modèles.

Dans l'exemple, on a \(p=14\) variables de base en incluant les
indicatrices pour les variables catégorielles multi-niveaux (revenu
\texttt{x3} et région \texttt{x4}), donc \(2^{14} = 16384\) modèles.
...
\phantomsection\label{recherche-exhaustive}
Heureusement, il n'est pas nécessaire d'ajuster tous les modèles:
certains peuvent être éliminés d'emblée.
...
Implémenté avec critères d'information dans le paquet
\texttt{BranchGLM}.
\end{frame}

\begin{frame}[fragile]{Recherche exhaustive dans \textbf{R}}
\phantomsection\label{recherche-exhaustive-dans-r}
...
\phantomsection\label{mot-dordre}
...
\phantomsection\label{approximation}
...
quadratiques, et les interactions d'ordre deux, pour obtenir plus de
flexibilité:
...
\phantomsection\label{moduxe8le-complet-et-syntaxe-r}
\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\CommentTok{\# Dans R, x1:x2 crée une interaction entre deux variables}
...
coefficients pour 210 observations.
\end{frame}

\begin{frame}{Méthodes de sélection séquentielles}
\phantomsection\label{muxe9thodes-de-suxe9lection-suxe9quentielles}
Recherche exhaustive typiquement trop coûteuse.

On peut plutôt opter pour un algorithme heuristique:
...
\phantomsection\label{digression}
Si on ajuste tous les modèles à \(K\) variables, celui qui a la plus
petite erreur moyenne quadratique (EQM) a aussi le plus petit \(R^2\),
\(\mathsf{AIC}\) et \(\mathsf{BIC}\) parmi ces modèles (puisque le
nombre de coefficients est le même).

Il suffit donc de rapporter le meilleur modèle avec \(1,2,\ldots, m\)
variables explicatives et choisir ensuite.
\end{frame}

\begin{frame}{Sélections ascendante et descendante}
\phantomsection\label{suxe9lections-ascendante-et-descendante}
On sélectionne un critère à minimiser (AIC/BIC) et une règle d'arrêt.

\textbf{Sélection ascendante} (\emph{forward selection}): à partir du
modèle de base (ordonnée à l'origine), ajouter à chaque étape au modèle
précédent la variable qui réduit le plus notre critère d'optimisation.

\textbf{Sélection descendante} (\emph{backward selection}): à partir du
modèle de départ (typiquement le modèle complet), enlever la variable
qui mène à la plus grande réduction du critère d'optimisation.

Dans les deux cas, la procédure se termine quand on ne peut plus réduire
le critère.

On choisit le modèle final parmi ceux considérés avec le plus petit
critère.
\end{frame}

\begin{frame}{Sélection séquentielle (\emph{stepwise})}
\phantomsection\label{suxe9lection-suxe9quentielle-stepwise}
...
\phantomsection\label{code-r-avec-leaps}
\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\CommentTok{\# Cette procédure séquentielle retourne}
\CommentTok{\# la liste de modèles de 1 variable à}
...
\NormalTok{  leaps}\SpecialCharTok{::}\FunctionTok{regsubsets}\NormalTok{(}
    ...
    \AttributeTok{nvmax =} \FunctionTok{length}\NormalTok{(}\FunctionTok{coef}\NormalTok{(mod\_complet))) }\CommentTok{\# ici, 105}
...
d'information}
\phantomsection\label{suxe9lection-suxe9quentielle-avec-crituxe8res-dinformation}
\footnotesize
...
\phantomsection\label{performance-en-fonction-de-la-complexituxe9}
\begin{center}
\includegraphics[width=0.9\linewidth,height=\textheight,keepaspectratio]{figures/fig-perfo-sequentiel.pdf}
\end{center}

\footnotesize

Performance des 40 premiers modèles de la procédure séquentielle en
fonction du nombre de termes inclus. La méthode externe donne la
performance sur les 100K données de validation (oracle).

\normalsize
\end{frame}

\begin{frame}{Méthodes de régularisation}
\phantomsection\label{muxe9thodes-de-ruxe9gularisation}
Objectif: prévenir le surajustement.

Si \(Y\) est l'observation, \(\widehat{Y}\) la prédiction et
\(\mathsf{E}(Y)\) la vraie moyenne, l'erreur moyenne quadratique se
décompose

\[\underset{\mathsf{EQM}}{\mathsf{E}\left[(Y-\widehat{Y})^2\right]} =  \underset{\text{biais carré}}{\mathsf{E}\left[\{\widehat{Y}-\mathsf{E}(Y)\}^2\right]} + \underset{\text{variabilité}}{\mathsf{E}\left[\{Y-\mathsf{E}(Y)\}^2\right]}\]

Les méthodes de régularisation introduisent du biais dans l'estimation
des coefficients en pénalisant leur norme.
\end{frame}

\begin{frame}{Compromis biais et variance}
\phantomsection\label{compromis-biais-et-variance}
\pandocbounded{\includegraphics[keepaspectratio]{MATH60602-diapos4_files/figure-beamer/unnamed-chunk-5-1.pdf}}
\end{frame}

\begin{frame}{Compromis biais variance}
\phantomsection\label{compromis-biais-variance}
\begin{figure}

\centering{

\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos4_files/figure-beamer/fig-surajustement-1.pdf}

}

\caption{\label{fig-surajustement}Illustration du compromis biais
(gauche) et variance (droite). À mesure que l'on complexifie le modèle,
l'ajustement s'améliore, mais le bruit finit par dominer.}

\end{figure}%
\end{frame}

\begin{frame}{Invariance des coefficients}
\phantomsection\label{invariance-des-coefficients}
Le modèle linéaire est invariant aux transformations affines.

Considérons un modèle dans lequel on changerait l'unité de la variable
\(\mathsf{x}_1\)

Si le coefficient estimé \(\widehat{\beta}_1=0.1\) quand
\(\mathsf{x}_1\) est la distance en km, alors on aura
\(\widehat{\beta}^\star_1=100 \times \widehat{\beta}_1 = 10\) si on
remplace cette variable par
\(\mathsf{x}^{\star}_1 = \mathsf{x}_1 / 100\) pour la distance exprimée
en tranche de 100km.

La prédiction ne change pas car
\(\widehat{\beta}_1\mathsf{x}_1=\widehat{\beta}^\star_1\mathsf{x}^{\star}_1\).
\end{frame}

\begin{frame}[fragile]{Préalable à la régression avec régularisation}
\phantomsection\label{pruxe9alable-uxe0-la-ruxe9gression-avec-ruxe9gularisation}
On pénalise la norme de \(\beta_1, \ldots, \beta_p\) (l'ordonnée à
l'origine n'est pas incluse).

Pour éviter que les échelles des variables (transformations affines)
mènent à des changements, on standardise préalablement les variables
explicatives \(\mathrm{X}_1, \ldots, \mathrm{X}_p\) \textbf{et} la
variable réponse \(\boldsymbol{y}\) pour qu'elles soient toutes à la
même échelle (par exemple, avec moyenne nulle et un écart-type
unitaire).

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\FunctionTok{scale}\NormalTok{(dbm\_a)}
\end{Highlighting}
\end{Shaded}

Cette étape peut-être effectuée implicitement par votre fonction ou
logiciel (c'est le cas avec \texttt{glmnet} dans \textbf{R}).
\end{frame}
...
L'hyperparamètre \(\lambda>0\) détermine la force de la pénalisation.

\begin{itemize}
\tightlist
\item
  Si \(\lambda=\infty\), la solution est de prendre
  \(\beta_1 = \cdots = \beta_p=0\) et on recouvre le modèle pour la
  moyenne globale, qui contient uniquement l'ordonnée à l'origine.
\item
  Si \(\lambda=0\), on obtient le même résultat que le modèle linéaire
  avec les \(p\) variables explicatives.
\end{itemize}

Le LASSO rétrécit de certains coefficients \textbf{exactement} à zéro:
sélection implicite de variable.
\end{frame}

\begin{frame}{Contrainte budgétaire et moindres carrés}
\phantomsection\label{contrainte-budguxe9taire-et-moindres-carruxe9s}
\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos4_files/figure-beamer/unnamed-chunk-7-1.pdf}
\end{center}
\end{frame}

\begin{frame}[fragile]{Code \textbf{R} pour le LASSO}
\phantomsection\label{code-r-pour-le-lasso}
...
\phantomsection\label{trajectoire-lasso}
On choisit typiquement \(\lambda\) par validation croisée, soit

\begin{itemize}
\tightlist
\item
  le modèle avec la plus petite valeur de
  \(\mathsf{EQM}_{\mathrm{VC}}\).
\item
  le modèle le plus parsimonieux à au plus un erreur-type de ce dernier
  (pénalité plus élevée, plus grand nombre de coefficients nuls)
\end{itemize}

\begin{center}
\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{figures/fig-lassopath.pdf}
\end{center}
\end{frame}

\begin{frame}{Paramètres}
\phantomsection\label{paramuxe8tres}
\begin{figure}

\centering{

\includegraphics[width=0.8\linewidth,height=\textheight,keepaspectratio]{MATH60602-diapos4_files/figure-beamer/fig-lassopath-1.pdf}

}

\caption{\label{fig-lassopath}Valeur des coefficients du modèle en
fonction de la pénalité (avec nombre de coefficients non-nuls sur le
dessus).}
...
\phantomsection\label{uxe9valuation-et-pruxe9diction}
Une fois la valeur de \(\lambda\) choisie, on ré-estime le modèle avec
la pénalité.
...
\phantomsection\label{pruxe9dictions}
...
\begin{frame}{Critères objectifs}
\phantomsection\label{crituxe8res-objectifs}
Il est important de distinguer entre la fonction objective (ou la
fonction de coût) utilisée pour l'estimation des paramètres, comme
...
\item
  le négatif de la log vraisemblance pour les modèles linéaires
  généralisés.
\end{itemize}

et le critère visant à choisir le meilleur modèle (par ex., le profit).
\end{frame}

\begin{frame}{Évaluation de la performance}
\phantomsection\label{uxe9valuation-de-la-performance}
...
15 & 13.64 & ridge, validation croisée avec 10 groupes \\
30 & 11.94 & LASSO, validation croisée avec 10 groupes \\
\bottomrule\noalign{}
\end{longtable}
\end{frame}

\begin{frame}{Récapitulatif}
\phantomsection\label{ruxe9capitulatif}
\begin{itemize}
\tightlist
\item
...
\begin{frame}{Récapitulatif}
\phantomsection\label{ruxe9capitulatif-1}
\begin{itemize}
\tightlist
\item
...
\begin{frame}{Récapitulatif}
\phantomsection\label{ruxe9capitulatif-2}
\begin{itemize}
\tightlist
\item
...
\item
  ...
  d'optimisation (séparation et évaluation) pour un nombre réduit de
  variables (max 50).
\item
  Sinon, on a l'option d'utiliser un algorithme heuristique qui ne
  couvre qu'un sous-ensemble de tous les modèles
\item
...
\begin{frame}{Récapitulatif}
\phantomsection\label{ruxe9capitulatif-3}
...
\tightlist
\item
  introduction de biais pour les coefficients (vers zéro) pour réduire
  la variabilité des estimations.
\item
  contrairement aux critères d'information (qui pénalisent l'ajustement
  a posteriori), les estimations des coefficients changent.
\end{itemize}
...
\tightlist
\item
  Cela correspond implicitement à une sélection de variables, bien que
  les estimations sont néanmoins différentes de celle des moindres
  carrés (régression linéaire) pour les mêmes variables explicatives.
\end{itemize}
\end{frame}

\begin{frame}{Récapitulatif}
\phantomsection\label{ruxe9capitulatif-4}
\begin{itemize}
\tightlist
\item
...
\item
  Faire un tri permet d'isoler certains candidats, pour lesquels on peut
  calculer notre critère de sélection.
\item
  On retient alors celui qui donne la meilleure performance.
\end{itemize}
\end{frame}
