...
Exemple de base de données marketing. Cibler les clients pour l'envoi d'un échantillon promotionnel et d'un catalogue de produits.

But: maximiser les levées de fonds

1. Envoyer un échantillon de produits au coût de 10$ à un groupe pilote de 1000 personnes et regarder le montant d'achat suite à l'envoi.
...
Le modèle utilise les données des 210 personnes (sur 1000) de l'échantillon d'apprentissage qui ont acheté suite à l'envoi du catalogue.
...
**Sélection ascendante** (*forward selection*): à partir du modèle de base (ordonnée à l'origine), ajouter à chaque étape au modèle précédent la variable qui réduit le plus notre critère.


**Sélection descendante** (*backward selection*): à partir du modèle de départ (typiquement le modèle complet), enlever la variable qui mène à la plus grande réduction du critère
...
plot(lasso_path, xlab = "valeur de la pénalité (norme L1)")
```
