...
Chaque variable est incluse (ou pas): il y a $2^p=2\times 2 \times \cdots \times 2$ ($p$ fois) modèles. Au total, $2^{14} = 16384.$
...
\footnotesize 
Performance des 40 premiers modèles de la procédure séquentielle en fonction du nombre de termes inclus. La méthode externe donne la vrai performance sur les 100K données de validation (oracle). 
...
**Solution**: standardiser les variables explicatives $\mathrm{X}_1, \ldots, \mathrm{X}_p$ **et** la variable réponse $\boldsymbol{y}$ pour qu'elles soient toutes à la même échelle (par exemple, avec moyenne nulle et un écart-type unitaire).

- vérifier selon le logiciel, cette étape peut-être effectuée implicitement par votre fonction.
...
et le critère visant à choisir le meilleur modèle (le profit en entreprise).
